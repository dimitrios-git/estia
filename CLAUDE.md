...
- **`custom/weather`** (`user/waybar/scripts/weather.sh`, symlinked) shows the condition emoji + temperature in the bar (color emoji — needs `fonts-noto-color-emoji`, installed; the whole bar string is wrapped in a `<span rise='…'>` to drop emoji+temp together onto the other modules' text baseline, since the emoji's tall glyph otherwise rides the row too high — a hand-tuned offset) and location/condition/feels-like/humidity/wind in the tooltip, from **wttr.in** (`curl`, **IP-geolocated** — no location configured). Refreshes every **30 min** (`interval` — wttr.in etiquette) and **caches** the last good reading to `$XDG_RUNTIME_DIR/waybar-weather.json`, falling back to it on a network failure (prints nothing → module hidden if there's no data at all). `weather.sh show` (the on-click) opens the full forecast.
...
- **Built-in tooltips:** `memory` hover shows used/total/free + swap (`tooltip-format` with `{avail}`/`{swap*}`); `clock` hover shows the full date (`%A, %d %B %Y`) + a **native month calendar** (`{calendar}` in `<tt><small>`, today accented `#ce0056`) — waybar v0.12, no external `cal`. Keep the calendar inside `<tt><small>` — the calendar rendering is finicky (it went blank when that wrapper was changed); the `actions` (`on-scroll` shift month) also enable hover-scroll month-flipping. The scroll brings a hand cursor with no click bound (a year-view `on-click` is a TODO once the calendar markup is confirmed stable).
...
- `custom/cpu`: nothing beyond coreutils (`/proc/stat` + sysfs hwmon). `custom/weather`: `curl` (in the manifest) + `fonts-noto-color-emoji` (installed) for the condition emoji; network for live data (caches otherwise).
