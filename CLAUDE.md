...
- **Click handlers** open floating TUIs as toggles (`pgrep … && pkill … || kitty --class floatterm -e …`) so a second click closes the popup:
  ...
  - `custom/gpu` → toggles a live GPU monitor (`gpu-watch.sh`) in a floatterm.
- **`custom/gpu`** is backed by `user/waybar/scripts/gpu.sh` (emits JSON): utilisation % + icon in the bar, card name / temp / VRAM in the tooltip. The script is **symlinked** into `~/.config/waybar/scripts/` (in the manifest) and `exec` references it via **`$HOME`** — waybar runs `exec`/`on-click` through `sh -c`, so the shell expands it (path generalisation, `docs/repo-structure-design.md` §5; no clone-absolute path, no manual edit on move). **Multi-vendor + self-hiding:** it tries **NVIDIA** (`nvidia-smi`), then **AMD** (amdgpu **sysfs** — `gpu_busy_percent`/hwmon/`mem_info_vram_*`, no extra tool), then **Intel** (sysfs, best-effort — i915 has no util counter, so name+temp only); priority NVIDIA > AMD > Intel on a multi-GPU box. With no supported GPU it prints nothing, so Waybar hides the whole module (icon included), keeping the config portable. The **`on-click`** runs the companion **`gpu-watch.sh`** (also symlinked), which picks the best live monitor available — `nvidia-smi`/`amdgpu_top`/`radeontop`/`intel_gpu_top`, falling back to a no-extra-tool sysfs poll — so the click works on any GPU; the toggle matches on the `gpu-watch.sh` path.
