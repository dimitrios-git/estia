...
| less / man | `LESS`/`MANPAGER` `-D` colours (`.bashrc`) | ✅ | `--use-color` `-D` classes on the palette: bold→accent red (man headers/commands), underline→blue (args), search→black-on-yellow, line-nums→grey, errors→bright red. (Fixed a `$D` bug that had dropped the underline colour in interactive less.) **`more`** has no colour options (n/a); **`systemctl`** is already on-palette — `SYSTEMD_COLORS` only toggles colour on/off, its semantic green/red render via the terminal (n/a) |
