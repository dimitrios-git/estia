...
- **`system/`** — layer-(a) **system configs** deployed by **copy**/template to root-owned locations (**not** symlinked), driven by Ansible roles: `system/samba/smb.conf` → `/etc/` (`samba` role); `system/sway-session/start-sway` → `/usr/local/bin/` (`sway_session` role). Each `system/` subdir keeps a `README.md` runbook.
