...
| Icons (app / folder / places) | Yaru (Suru) icon theme | 🟡 | **`yaru_icons` role** (opt-in `enable_yaru_icons`, default off) builds a custom **`hestia`** accent flavour from pinned Yaru source → installs **`Yaru-hestia`** in `~/.local/share/icons`. No sed-shortcut (Yaru folders are PNGs), so it renders via meson/ninja/inkscape under xvfb — heavy. Accent is `optimize-contrast()`-adjusted, not byte-exact. 🟡 until a **live build + on-target verification** confirms the render/contrast/inherits |
