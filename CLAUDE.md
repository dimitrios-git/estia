...
- **Audit for untracked dependencies.** `apt_packages` (`bootstrap/group_vars/all.yml`) is explicitly *not exhaustive*. Sweep every tool config + the docs for required packages/binaries that aren't yet tracked (in `apt_packages`, or the README/runbook **manual** list for non-apt ones). Known gaps to start from: the **Nerd Fonts** — `BigBlueTerm437 Nerd Font Mono` (waybar) and `Lilex Nerd Font Mono` (kitty), installed manually to `~/.local/share/fonts` + `fc-cache` (not in apt); a **C compiler** (`gcc`/`build-essential`) for nvim treesitter; **`xwayland`** for sway X apps; **`libreoffice`** for vifm's office-doc opens. Add the apt-able ones to the right `apt_packages` group (mind the feature-gating), document the rest as manual.
- **Investigate SSH terminal key weirdness** (backspace inserts a space, garbled editing when SSH'd into a server). Leading suspect is **not** `set -o vi` in `user/bash/.bashrc` — that's *local* readline; the remote shell is separate — but the **kitty terminfo**: kitty sets `TERM=xterm-kitty`, which most servers don't have, so they mis-handle backspace/arrows. Fix candidates: `kitty +kitten ssh <host>` (auto-installs the terminfo on the remote), copy the `xterm-kitty` terminfo to the servers, or export `TERM=xterm-256color` for ssh. Confirm whether vi-mode contributes before ruling it out.
