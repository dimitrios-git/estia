...
- **Modules** (right side): `custom/cpu`, `memory`, `custom/gpu0`/`gpu1`/`gpu2` (one per GPU — see below), `power-profiles-daemon`, `pulseaudio`, `user/sway/language`, `network`, `bluetooth`, `custom/notifications`, `custom/weather`, `clock`, `custom/debian`. Right-side modules share styling via a grouped CSS selector — add new ones to that group.
