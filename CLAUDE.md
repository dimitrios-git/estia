...
- **Rendered (not symlinked)** — a small **path-generalised / identity-bearing** set rendered from `.j2` by the bootstrap (`git/.gitconfig`, `glow/glow.yml`, `cmus/rc`, and the two credential scripts `credential-unlock.sh` + `keyring-ssh-askpass.sh`); see *Rendered (templated) configs* below.
...
| `gnupg/credential-unlock.sh.j2` | `~/.config/sway/scripts/credential-unlock.sh` |
| `gnupg/keyring-ssh-askpass.sh.j2` | `~/.config/sway/scripts/keyring-ssh-askpass.sh` |
...
A fresh machine is reproduced by an **Ansible** playbook (engine decision: `docs/repo-structure-design.md`). The one-shot entry point is **`bootstrap/setup.sh`** — installs Ansible, interactively gathers host answers (the §6 feature toggles + `samba_lan_subnet` + `cmus_music_dir`, with auto-detected defaults) into the untracked `host_vars/localhost.yml`, then runs the playbook (re-runnable; extra args pass through). The ordered start-to-finish narrative — which role when, with the interactive/external manual steps interleaved — is **`docs/install-runbook.md`**; the per-role reference is `bootstrap/README.md`.
...
- `glow.yml` sets the default `style:` to `wildcharm.json` **by absolute path** — glow can't expand `~`/`$VARS` there. So it's **path-generalised** (not symlinked): the repo carries `glow/glow.yml.j2`, and the `dotfiles` role renders it to `~/.config/glow/glow.yml` with the path templated from `{{ target_home }}` (`docs/repo-structure-design.md` §5; edit the `.j2` + re-run the role to apply).
...
- **`custom/gpu`** is backed by `waybar/scripts/gpu.sh` (runs `nvidia-smi`, emits JSON): utilisation % + icon in the bar, card name / temp / VRAM in the tooltip. The script is **symlinked** into `~/.config/waybar/scripts/` (in the manifest) and `exec` references it via **`$HOME`** — waybar runs `exec`/`on-click` through `sh -c`, so the shell expands it (path generalisation, `docs/repo-structure-design.md` §5; no clone-absolute path, no manual edit on move). **NVIDIA-only and self-hiding:** the script guards on `command -v nvidia-smi` and prints nothing when it (or a card) is absent, so Waybar hides the whole module — icon included — on non-NVIDIA hosts, keeping the config portable. AMD/Intel aren't supported (different tooling: `amdgpu_top`/sysfs, `intel_gpu_top`); making the script vendor-agnostic under the same `custom/gpu` id is a possible future extension.
