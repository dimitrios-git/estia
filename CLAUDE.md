...
- **Collaboration is git-mediated, two principals.** claude commits and signs as **itself**, pushes a topic branch, and opens a **pull request** as the GitHub bot **`dimitrios-claude`** (`gh pr create`; commits show **Verified**); dimitrios reviews, **merge-commits** (preserves the signed commits), and pulls to deploy. claude **never** self-merges, and never co-edits the human's clone. The concrete loop (and the `gh`/PAT auth) is in `docs/working-with-claude.md`.
